...
func (r *vrackServicesResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Error waiting Put %s result", endpoint),
			err.Error(),
		)
		return
	}
...
// Wait for asynchronous processing is done and get last resources state.
func (r *vrackServicesResource) WaitForUpdate(ctx context.Context, resourceName string) (*VrackServicesModel, error) {
	log.Printf("[DEBUG] WaitForUpdate on %s", resourceName)

	endpoint := "/v2/vrackServices/resource/" + url.PathEscape(resourceName)

	stateConf := &retry.StateChangeConf{
		Pending: []string{"UPDATING"},
		Target:  []string{"READY"},
		Refresh: func() (interface{}, string, error) {
			res := &VrackServicesModel{}
			endpoint := endpoint
			err := r.config.OVHClient.GetWithContext(ctx, endpoint, res)
			if err != nil {
				return res, "", err
			}
			status := res.ResourceStatus.ValueString()
